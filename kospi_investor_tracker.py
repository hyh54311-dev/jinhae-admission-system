...
def send_telegram(msg):
    ...
    import sys
    if "--test" in sys.argv:
        print("[TEST MODE] Skip calling Telegram API as per Rule 6.")
        return
...
def main():
    ...
    try:
        ...
        try:
            ...
            if "--test" in sys.argv:
                print(f"[TEST MODE] Index fetch failed ({e}). Using mock index data.")
                index_history = [{
        ...
        try:
            ...
            if "--test" in sys.argv:
                print(f"[TEST MODE] Investor fetch failed ({e}). Using mock investor data.")
        ...
        # Calculate 5-day (1-week), 20-day (1-month) and 120-day (6-month) cumulative trends
        trend_5 = analyze_cumulative_trend(investor_df, 5)
        trend_20 = analyze_cumulative_trend(investor_df, 20)
        ...
        if trend_5 and trend_20 and trend_120:
            msg_lines.extend([
                "📅 <b>기간별 누적 매매동향</b>",
                f"• 1주일(5일): 개인 <code>{format_amount(trend_5['individual'])}</code> | 외인 <code>{format_amount(trend_5['foreigner'])}</code> | 기관 <code>{format_amount(trend_5['institution'])}</code>",
                f"• 1달(20일): 개인 <code>{format_amount(trend_20['individual'])}</code> | 외인 <code>{format_amount(trend_20['foreigner'])}</code> | 기관 <code>{format_amount(trend_20['institution'])}</code>",
