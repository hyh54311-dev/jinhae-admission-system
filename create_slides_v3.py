...
BG_GYUWONGA = r"C:\Users\admin\.gemini\antigravity\brain\541b9e54-1667-4acf-816f-988959751015\bg_gyuwonga_dim.png"
BG_INHYEON = r"C:\Users\admin\.gemini\antigravity\brain\541b9e54-1667-4acf-816f-988959751015\bg_inhyeon_dim.png"

# ?숈깮?ㅼ뿉寃?移쒖닕?섍퀬 紐⑤뜕???고듃 ?좊떦 (援ш? ?щ씪?대뱶 ?곕룞 怨좊젮)
TITLE_FONT = "Gmarket Sans Medium" 
BODY_FONT = "Noto Sans KR"

def dim_image(src, dst):
    try:
        img = Image.open(src).convert("RGB")
        black = Image.new('RGB', img.size, 'black')
        # ?щ챸??50% ?섏??먯꽌 寃??됯낵 釉붾젋?⑺븯???꾩쟾??媛?낆꽦 1?쒖쐞濡?留뚮벀
        dimmed = Image.blend(img, black, 0.6) 
        dimmed.save(dst)
...
def add_custom_slide(prs, bg_img_path, title_text, bullet_points):
    ...
    p_title.font.color.rgb = RGBColor(255, 230, 0)
    ...
    first_p.font.color.rgb = RGBColor(255, 130, 130) if first_pt[2] else RGBColor(255, 255, 255)
    ...
    for pt in bullet_points[1:]:
        ...
        p.font.color.rgb = RGBColor(255, 130, 130) if highlight else RGBColor(255, 255, 255)
        if highlight:
            p.font.bold = True
            
def create_presentation():
    # 諛곌꼍 ?대?吏 ?ㅽ겕??(媛?낆꽦???꾪븳 ?щ챸??議곗젅 ?④낵)
    dim_image(BG_GYUWONGA_RAW, BG_GYUWONGA)
    dim_image(BG_INHYEON_RAW, BG_INHYEON)
    ...
    prs.save("worksheet_presentation_v3.pptx")
    print("Local V3 presentation saved as worksheet_presentation_v3.pptx")
...
def upload_presentation():
    ...
    file_metadata = {
        'name': '[?щ씪?대뱶 V3] 2027 ?섎뒫?밴컯 臾명븰 (?고듃 由щ돱??& ?λ툝??諛깃렇?쇱슫??',
        'parents': [FOLDER_ID],
        'mimeType': 'application/vnd.google-apps.presentation'
    }
    
    media = MediaFileUpload('worksheet_presentation_v3.pptx',
