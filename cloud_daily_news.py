...
def call_gemini_api(prompt):
    ...
    try:
        ...
        if response.status_code != 200:
            log_message(f"API 에러 발생 (Status {response.status_code})")
            log_message(f"응답 본문: {response.text}")
        ...
        if not candidates:
            log_message(f"오류: API 응답에 candidates가 없습니다 (세이프티 필터링 가능성): {json_data}")
            return None
            
        # 첫 번째 candidate의 content.parts 확인
        ...
        if not result_text:
             log_message(f"주의: 텍스트 응답이 비어있습니다. (finishReason: {candidate.get('finishReason')})")
...
def generate_weekend_news():
    try:
        ...
        if content:
            ...
            log_message("콘텐츠 생성 실패로 텔레그램 메시지를 전송하지 않았습니다.")
            send_telegram_message(f"⚠️ 오류: {today_str_full} 주말 뉴스 생성에 실패했습니다. GitHub Actions 로그를 확인해 주세요.")
