...
function refreshAttendanceSystem() {
  ...
  sheets.forEach(function(sheet) {
    ...
    if (sheetName.includes("반")) {
      cleanUpUnusedColumnsInSheet(sheet); // 1. 과거 열 정리
      addTodayColumn(sheet);              // 2. 오늘 열 생성 및 스마트 기본값 입력
      SpreadsheetApp.flush();             // 구글 시트 확실한 강제 새로고침(동기화)
      applyDropdownRetroactively(sheet);  // 3. 전체 드롭다운 최신화
    }
  });
}
...
// Module exports for the test harness running on Node.js
