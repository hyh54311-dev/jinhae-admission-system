...
def call_gemini_api(prompt):
    ...
    try:
        ...
        if response.status_code != 200:
            error_msg = f"API 에러 발생 (Status {response.status_code}): {response.text}"
            log_message(error_msg)
            send_telegram_message(f"⚠️ 주말 뉴스 자동화 API 오류:\n{error_msg}")
        ...
        # 오류 상세 내용을 텔레그램으로 전송하여 GitHub에 들어오지 않고도 원인을 파악할 수 있도록 함
        send_telegram_message(f"⚠️ 주말 뉴스 자동화 실행 중 예외 발생:\n{str(e)}")
...
def generate_weekend_news():
    ...
    if content:
        ...
        # call_gemini_api 내에서 이미 텔레그램 알림을 보내도록 수정했으므로, 여기서는 로그만 기록해도 무방합니다.
        log_message(f"오류: {today_str_full} 주말 뉴스 생성에 실패했습니다.")
