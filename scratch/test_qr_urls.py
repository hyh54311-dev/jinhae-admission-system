...
for label, url in urls_to_test:
    try:
        ...
        if status in (200, 301, 302):
            results.append((label, url, status, "✅ 성공 (정상 접속 가능)"))
        else:
            results.append((label, url, status, f"❌ 실패 (HTTP 상태 코드: {status})"))
    except Exception as e:
        results.append((label, url, "ERR", f"❌ 통신 에러: {e}"))

for label, url, status, result_str in results:
    print(f"[{label}] -> Status: {status} | {result_str}")
