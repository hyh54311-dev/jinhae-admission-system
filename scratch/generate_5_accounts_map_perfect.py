...
# 타이틀
ax.text(50, 94, "주식·ETF 투자 5대 핵심 계좌 한눈에 보기", fontsize=20, fontweight='bold', ha='center', color='#1e293b')
ax.text(50, 88, "내 투자 목적과 기간에 딱 맞는 최적의 계좌를 3초 만에 선택하세요!", fontsize=12, ha='center', color='#64748b')
...
cards = [
    {
        ...
        "color": "#f8fafc",
        "header_color": "#475569",
        "desc": ["• 모든 국내/해외 주식 자유 거래", "• 양도세 22% (해외 250만 원 공제)", "• 입출금 제약 제로 (자유 거래)"],
        "x": 3, "y": 48, "w": 28, "h": 34
    },
    {
        "title": "2. ISA 중개형 계좌",
    ...
    },
    {
        ...
        "color": "#f0fdf4",
        "header_color": "#16a34a",
        "desc": ["• 연 600만 원 세액공제 환급금", "• 건강보험료 0.0% (전액 100% 제외)", "• 장기 복리 스노우볼 결실"],
        "x": 69, "y": 48, "w": 28, "h": 34
    },
    {
        ...
        "color": "#fffbeb",
        "header_color": "#d97706",
        "desc": ["• 세액공제 한도 900만 원 확장", "• 안전자산 30% 의무 보유", "• 퇴직금 이체 및 은퇴 준비"],
        "x": 19.5, "y": 8, "w": 28, "h": 34
    },
    {
        "title": "5. CMA 파킹 계좌",
        "sub": "단 하루만 맡겨도 매일 이자",
...
for card in cards:
    ...
    ax.text(card["x"] + card["w"]/2, card["y"] + card["h"] - 8.5, card["sub"],
            fontsize=9.5, ha='center', color='#334155')
    
    # 태그
    tag_rect = patches.FancyBboxPatch((card["x"] + 3, card["y"] + card["h"] - 14), card["w"] - 6, 4,
                                       boxstyle="round,pad=0.5,rounding_size=1",
                                       linewidth=0, facecolor=card["header_color"])
    ax.add_patch(tag_rect)
    ax.text(card["x"] + card["w"]/2, card["y"] + card["h"] - 12, card["tag"],
            fontsize=9.5, fontweight='bold', ha='center', color='#ffffff')
    
    # 설명 Bullet points
    for idx, d in enumerate(card["desc"]):
        ax.text(card["x"] + 2.5, card["y"] + card["h"] - 19 - (idx * 4.5), d,
...
print(f"SUCCESSFULLY GENERATED PERFECT 5 ACCOUNTS MAP AT {chart_path}")
