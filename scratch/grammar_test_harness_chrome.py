...
def main():
    ...
    with sync_playwright() as p:
        # User Chrome data profile launch or fallback
        try:
            ...
            print("Chrome 사용자 프로필 모드로 실행 완료!")
        except Exception as e:
            print("Chrome 프로필 실행 실패 (Chrome 사용 중인 경우), 표준 브라우저로 폴백:", e)
            browser = p.chromium.launch(headless=True)
            context = browser.new_context()

        page = context.pages[0] if context.pages else context.new_page()
        page.goto(url)
        print("페이지 및 Apps Script 프레임 로딩 대기 중 (8초)...")
        time.sleep(8)
        ...
        if not target_frame:
            print("⚠️ google.script.run 프레임을 찾지 못해 전체 프레임 스캔...")
            for f in page.frames:
        ...
        if target_frame:
            ...
            print("❌ Google Apps Script 실행 프레임을 탐색하지 못했습니다. 웹앱 배포 설정에서 '액세스할 수 있는 사용자: 모든 사용자(Anyone)'로 변경되어 있는지 확인해 주세요.")
