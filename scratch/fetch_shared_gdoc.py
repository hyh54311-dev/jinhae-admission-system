...
print(f"Fetching updated Google Doc TXT Export for ID: {doc_id}...")

try:
    req = urllib.request.Request(txt_url, headers={'User-Agent': 'Mozilla/5.0'})
    with urllib.request.urlopen(req) as resp:
        content = resp.read().decode('utf-8', errors='ignore')
        print(f"=== Content Successfully Fetched! (Total Length: {len(content)} chars) ===")
        with open(r"g:\다른 컴퓨터\내 컴퓨터\진해고등학교\2026학년도\antigravity_folder\scratch\gdoc_170cd_shared.txt", "w", encoding='utf-8') as f:
            f.write(content)
