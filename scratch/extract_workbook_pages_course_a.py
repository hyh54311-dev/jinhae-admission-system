...
def main():
    ...
    # Using the public account for Course A
    username = "trust_ai"
    password = "dong001!!"
    ...
    with sync_playwright() as p:
        ...
        # Get count of tabs dynamically
        count = wb_frame.locator(sidebar_tabs_selector).count()
        print(f"Detected {count} tabs in the workbook.")
        ...
        for idx in range(count):
            ...
            try:
                ...
                screenshot_path = os.path.join(screenshot_dir, f"course_a_page_{idx+1}.png")
                page.screenshot(path=screenshot_path)
                print(f"Saved screenshot: {screenshot_path}")
                
                # Save frame HTML
                html_path = os.path.join(screenshot_dir, f"course_a_page_{idx+1}.html")
                ...
                err_screenshot = os.path.join(screenshot_dir, f"course_a_error_tab_{idx+1}.png")
                page.screenshot(path=err_screenshot)
                print(f"Saved error screenshot: {err_screenshot}")
                
        print("\nCourse A workbook pages extraction complete!")
        browser.close()
