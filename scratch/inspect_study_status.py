...
df = pd.read_excel(file_path, sheet_name='자습 현황 명렬')

print("Rows around '도움반' in '자습 현황 명렬':")
# Let's find where '도움반' is in the df
for r_idx in range(df.shape[0]):
    for c_idx in range(df.shape[1]):
        val = df.iloc[r_idx, c_idx]
        if pd.notna(val) and '도움' in str(val):
            print(f"Row {r_idx}, Col {c_idx}: {val}")
            # print rows around it
            start_row = max(0, r_idx - 2)
            end_row = min(df.shape[0], r_idx + 15)
            start_col = max(0, c_idx - 1)
            end_col = min(df.shape[1], c_idx + 3)
