...
ax.plot(df_monthly.index, df_monthly["Close"], color="#1b6ec2", linewidth=2.2, label="코스피(KOSPI) 지수 궤적")
...
ax.set_title("K-듀얼모멘텀 코스피(KOSPI) 구간별 매수/안전자산 피신 시각화 (2014~2026)", fontsize=16, fontweight="bold", pad=15)
...
ax.text(0.02, 0.92, textbox_text, transform=ax.transAxes, fontsize=11, fontweight="bold",
...
legend_elements = [
    plt.Line2D([0], [0], color='#1b6ec2', lw=2.2, label='코스피(KOSPI) 지수 궤적'),
    Patch(facecolor='#2b8a3e', edgecolor='none', alpha=0.3, label='코스피 매수 기간 (초록색 세로 음영)'),
...
art_path = os.path.join(art_dir, 'k_momentum_kospi_signal_chart.png')
shutil.copy(chart_path, art_path)

print(f"SUCCESSFULLY GENERATED CLEAN CHART AT {chart_path} AND {art_path}")
