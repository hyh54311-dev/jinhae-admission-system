...
def main():
    ...
    try:
        ...
        # NTFS Zone.Identifier 제거 (파일 보안 경고 해제 - Unblock)
        zone_id_path = docx_path + ":Zone.Identifier"
        if os.path.exists(zone_id_path):
            try:
                os.remove(zone_id_path)
                print("Successfully removed NTFS Zone.Identifier to unblock the file.")
            except Exception as zone_err:
                print(f"Failed to remove Zone.Identifier: {zone_err}")
        else:
            print("No NTFS Zone.Identifier found, file is already unblocked.")

        # 2. HWP COM 호출 (Visible=True로 진행)
        ...
        try:
            ...
            if not open_success:
                print("Failed to open with format 'DOCX'. Trying 'Microsoft Word'...")
                open_success = hwp.Open(docx_path, "Microsoft Word", "")
                
            if not open_success:
                print("Failed with formats. Trying default Open...")
                open_success = hwp.Open(docx_path)
                
            if open_success:
                hwp.SaveAs(hwp_path, "HWP")
                print("Successfully saved HWP file!")
                success = True
            else:
