...
if [ -z "$FUNC_INFO" ]; then
  echo "⚠️ Existing Cloud Function not found in this project."
  echo ">> We will initialize a new Cloud Function named 'rebalancing-bot' in region 'asia-northeast3' (Seoul)."
  FUNC_NAME="rebalancing-bot"
  FUNC_REGION="asia-northeast3"
else
  ...
  echo "✅ Found existing Cloud Function: '$FUNC_NAME' in region '$FUNC_REGION'"
fi
echo ""
...
def trigger_rebalancing(request):
    ...
    # 3. Apply overrides to environment variables if specified
    if mock_run != '':
    ...
    try:
        # Re-initialize configuration in kis_bot_multi dynamically based on overrides
        kis_bot_multi.init_config()
        
        print(">> GCP 서버리스 트리거 수신. 가동 모드:")
        print(f"   - Force Run: {force_run}")
        print(f"   - KIS_MOCK: {kis_bot_multi.KIS_MOCK}")
        print(f"   - KIS_DRY_RUN: {kis_bot_multi.KIS_DRY_RUN}")
        ...
        return {
            ...
            "config": {
                "force": force_run,
                "mock": kis_bot_multi.KIS_MOCK,
                "dry_run": kis_bot_multi.KIS_DRY_RUN
            }
        }, 200
        ...
        # Re-initialize configuration to restore original settings
        kis_bot_multi.init_config()
...
# 4. Deploy the Cloud Function (타임아웃 5분 및 메모리 512MB 확장 적용)
